<template>
  ...
  <div style="padding-top: calc((100vh - 70vh)/2)">
    ...
    <button v-on:click='sendData({type: "store_user"})'>Send data</button>
...
export default {
  ...
  methods: {
    ...
    sendData(data) {
      console.log(data);
      data.username = child_id      
     // webSocket.addEventListener('open',  (event) =>{
        console.log("hererereererereerereererer")
...
#aelo, #aeli, #aero, #aeri, #anm1, #anm2, #anm3{
  visibility:hidden;
  opacity: 0;
}

#selo, #seli, #sero, #seri, #sum1, #sum2, #sum3{
  visibility:hidden;
  opacity: 0;
}

#helo, #heli, #hero, #heri, #hm1, #hm2, #hm3{
  visibility:hidden;
  opacity: 0;  
}

#afero, #aferi, #afelo, #afeli, #afm1, #afm2, #afm3{
  visibility:hidden;
  opacity: 0; 
}
#diero, #dieri, #dielo, #dieli, #dim1, #dim2, #dim3{
  visibility:hidden;
  opacity: 0;  
}
#sadTear {
...
#saelo, #saeli, #saero, #saeri, #sm1, #sm2, #sm3{
  visibility:hidden;
  opacity: 0;
}
